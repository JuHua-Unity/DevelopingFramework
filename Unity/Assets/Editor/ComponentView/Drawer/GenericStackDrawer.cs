...
{
    [ComponentViewDrawer]
    internal class GenericStackDrawer : IComponentViewDrawer
    {
        ...
        public object DrawAndGetNewValue(Type type, object value, DrawInfo draw, FieldInfo field)
        {
            ...
            else
            {
                ...
                if (fold)
                {
                    ...
                    {
                        type.GetMethod("Push").Invoke(value, new object[1] { ComponentViewHelper.CreateInstance(type.GetGenericArguments()[0]) });
                    }
                    EditorGUILayout.EndHorizontal();
                }
