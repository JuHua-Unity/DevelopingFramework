...
{
    [ComponentViewDrawer]
    internal class GenericQueueDrawer : IComponentViewDrawer
    {
        ...
        public object DrawAndGetNewValue(Type type, object value, DrawInfo draw, FieldInfo field)
        {
            ...
            else
            {
                ...
                if (fold)
                {
                    ...
                    {
                        type.GetMethod("Enqueue").Invoke(value, new object[1] { ComponentViewHelper.CreateInstance(type.GetGenericArguments()[0]) });
                    }
                    EditorGUILayout.EndHorizontal();
                }
